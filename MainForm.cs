...
namespace RoomKeypadManager
{
    public partial class MainForm : Form
    {
        ...
        private void InitializeRoomKeypadManagerTab(TabPage roomKeypadTab)
        {
            ...
            {
                if (logListBox.Items.Count > 0) // ListBoxにアイテムがあるか確認
                {
                    // ListBoxのすべてのアイテムを改行区切りで結合
                    var logs = string.Join(Environment.NewLine, logListBox.Items.Cast<string>());

                    // クリップボードにコピー
                    Clipboard.SetText(logs);
                    MessageBox.Show("ログをクリップボードにコピーしました。", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("ログが空です。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            };
        ...
        private void CreateDebugEnvironmentButton_Click(object sender, EventArgs e)
        {
            ...
            // 分割パネルの作成 (左60%、右40%に分割)
            SplitContainer splitContainer = new SplitContainer
            {
                Dock = DockStyle.Fill,
                Orientation = Orientation.Vertical,
                IsSplitterFixed = false // 固定せず、動的調整を許可
            };

            // debugPanel のサイズが確定した後に SplitterDistance を設定
            splitContainer.SizeChanged += (sender, e) =>
            {
                splitContainer.SplitterDistance = (int)(splitContainer.Width * 0.6); // 左60%、右40%
            };
            ...
            foreach (var device in structuredData[selectedRoomKey].Where(d => d.Model.Contains("MWP-T-OHW")))
            {
                Panel designContainer = new Panel
                {
                    ...
                    Size = new Size(250, 250), // 固定サイズ
                    Margin = new Padding(10),
                    BorderStyle = BorderStyle.FixedSingle // ボーダーを付けて分かりやすく
                };
                ...
                for (int i = 0; i < 3; i++)
                {
                    designLayout.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 33.33f)); // 列幅を均等に
                }
                designLayout.RowStyles.Add(new RowStyle(SizeType.Absolute, 70)); // 上部ディスプレイ
                designLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 50)); // ボタンの1段目
                designLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 50)); // ボタンの2段目

                // ディスプレイ部分
                Panel displayArea = new Panel
                {
                    BackColor = Color.Gray,
                    Dock = DockStyle.Fill // フィル全体に拡張
                };
                designLayout.Controls.Add(displayArea, 0, 0);
                designLayout.SetColumnSpan(displayArea, 3);

                // ボタンエリア
                string[] buttonTexts = { "Pow", "℃/℉", "▲", "🌀", "", "▼" }; // ボタンのテキスト
                Font buttonFont = new Font(FontFamily.GenericSansSerif, 16, FontStyle.Bold); // ボタンのフォント
                ...
                for (int i = 0; i < 6; i++)
                {
                    Button button = new Button
                    {
                        ...
                        Text = buttonTexts[i], // ボタンにマークを設定
                        Dock = DockStyle.Fill,
                        Font = buttonFont,
                        Margin = new Padding(2) // ボタン間の余白を調整
                    };
                    designLayout.Controls.Add(button, i % 3, 1 + (i / 3)); // ボタンを適切に配置
                }
